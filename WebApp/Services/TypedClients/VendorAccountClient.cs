...
{
    public class VendorAccountClient : IVendorAccountClient
    {
        ...
        public VendorAccountClient(HttpClient client, ILogger<AuthenticateClient> logger, IHttpClientHelper clientHelper, IHttpContextAccessor context, ITokenManager tokenManager, IUserSessionManager sessionManager, IMapper mapper)
        {
            _client = client;
            _logger = logger;
        ...
        public async Task<string> ForgetPasswordAsync(string PhoneNumber,string For)
        {
            HttpContent Content = _clientHelper.CreateHttpContent<string>(PhoneNumber);

            HttpResponseMessage response = await _client.PostAsync($"VendorAccount/ForgetPassword/{PhoneNumber}/{For}", null);
        ...
        {
            HttpResponseMessage response = await _client.PostAsync($"VendorAccount/VerifyOTP/{OTP}/{UserId}", null);
        ...
        public async Task<bool> ResetPasswordAsync(ChangePasswordDTO ResetData)
        {
            ...
            HttpResponseMessage response = await _client.PostAsync("VendorAccount/ResetPassword", Content);
        ...
        public async Task<bool> ChangePasswordAsync(ChangePasswordViewModel changePasswordViewModel)
        {
            ...
            var HttpResponse = await _client.PostAsync("VendorAccount/ChangePassword", content);
        ...
        public async Task<LoginResponse> LoginAsync(string Phonenumber, string Password, bool RememberMe)
        {
            ...
            HttpResponseMessage response = await _client.PostAsync("VendorAccount/Login", Content);
        ...
        private async Task SignInAsync(LoginResponse response, bool RememberMe = false)
        {
            ...
            _sessionManager.SetVendorStore(response.Vendor);
