...
namespace WebApp.Areas.Garage.Controllers
{
    ...
    public class AccountController : Controller
    {
        ...
        public async Task<IActionResult> Login()
        {
            ...
            {
                if ((await _authorizationService.AuthorizeAsync(User, "GarageOwner")).Succeeded)
                {
                    return RedirectPermanent("/Client/Dashboard/Index");
                }
                else
                {
                    ...
                    {
                        if (role == Enum.GetName(Roles.GarageOwner))
                            return RedirectPermanent("/Client/Dashboard/Index");
                        else
                            return View();
                    }
        ...
        {
            await HttpContext.SignOutAsync(new AuthenticationProperties() { RedirectUri = "Garage" });
        ...
        public async Task<IActionResult> ForgetPassword(ForgotPasswordViewModel forgotPasswordViewModel)
        {
            ...
            try
            {
                ...
                if (ModelState.IsValid)
                {
                    string response = await _authService.ForgetPasswordAsync(forgotPasswordViewModel.Email, Enum.GetName(typeof(Logins), Logins.Garage));
                    if (response != null)
                    {
                        return Json(new
                        {
                            success = true,
                            url = "Client/Account/VerifyOTp?UserId=" + response + "&Email=" + forgotPasswordViewModel.Email,
                            message = "success"
                        });
                    }
                    else
        ...
        public async Task<IActionResult> ResendOTP(string Email)
        {
            ...
            try
            {
                ...
                if (ModelState.IsValid)
                {
                    string response = await _authService.ForgetPasswordAsync(Email, Enum.GetName(typeof(Logins), Logins.Garage));
                    if (response != null)
                    {
                        return Json(new
                        {
        ...
        public async Task<IActionResult> VerifyOTP(string UserId, string OTP)
        {
            try
            {
                ...
                return Json(new { success = true, message = "OTP Verified", url = "/Client/Account/ResetPassword?UserId=" + UserId });
            }
        ...
        public async Task<IActionResult> ResetPassword(ChangePasswordDTO resetPasswordViewModel)
        {
            try
            {
                ...
                if (Otp)
                {
                    return Json(new { success = true, message = "Password reset successfully", url = "/Client" });
                }
                else
                {
                    return Json(new { success = false, message = "Something went wrong" });
