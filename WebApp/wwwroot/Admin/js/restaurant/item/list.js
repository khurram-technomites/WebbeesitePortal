		...
		table1 = table.DataTable({
			...
			columnDefs: [{
				...
				render: function (data, type, full, meta) {
					...
					actions += '<a class="btn btn-bg-secondary btn-icon btn-sm mr-1" href="/Restaurant/Item/Edit?id=' + data[1] + '">' +
						'<i class="fa fa-pen"></i>' +
						'</a> ' +
						'<button type="button" class="btn btn-bg-secondary btn-icon btn-sm mr-1" onclick="OpenModelPopup(this,\'/Restaurant/Item/Details/' + data[1] + '\')" title="View">' +
...
function Delete(element, record) {
	...
	swal.fire({
		...
		if (result.value) {
			...
			$.ajax({
				url: '/Restaurant/Item/Delete/' + record,
				type: 'POST',
				data: {
...
function Activate(element, record) {
	swal.fire({
		...
		if (result.value) {
			...
			$.ajax({
				url: '/Restaurant/Item/ToggleActiveStatus/' + record,
				type: 'Get',
