		...
		table1 = table.DataTable({
			...
			columnDefs: [{
				...
				render: function (data, type, full, meta) {
					...
					actions += '<button type="button" class="btn btn-bg-secondary btn-icon btn-sm mr-1" onclick="OpenModelPopup(this,\'/Restaurant/RestaurantKitchenManager/Edit/' + data[1] + '\',true)">' +
						'<i class="fa fa-pen"></i>' +
						'</button> ' +
						'<button type="button" class="btn btn-bg-secondary btn-icon btn-sm mr-1" onclick="OpenModelPopup(this,\'/Restaurant/RestaurantKitchenManager/Details/' + data[1] + '\')" title="View">' +
...
function Delete(element, record) {
	...
	swal.fire({
		...
		if (result.value) {
			...
			$.ajax({
				url: '/Restaurant/RestaurantKitchenManager/Delete/' + record,
				type: 'POST',
				data: {
...
function Activate(element, record) {
	swal.fire({
		...
		if (result.value) {
			...
			$.ajax({
				url: '/Restaurant/RestaurantKitchenManager/ToggleActiveStatus/' + record,
				type: 'Get',
