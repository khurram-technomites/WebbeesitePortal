		...
		table1 = table.DataTable({
			...
			columnDefs: [{
				...
				render: function (data, type, full, meta) {
					...
					actions += '<button type="button" class="btn btn-bg-secondary btn-icon btn-sm mr-1" onclick="OpenModelPopup(this,\'/Admin/ClientTypes/Edit/' + data[1] + '\',true)">' +
						'<i class="fa fa-pen"></i>' +
						'</button> ' +
						'<button type="button" class="btn btn-bg-secondary btn-icon btn-sm mr-1" onclick="OpenModelPopup(this,\'/Admin/ClientTypes/Details/' + data[1] + '\')" title="View">' +
					...
					//	actions += '<button type="button" class="btn btn-outline-success btn-sm mr-1" onclick="Activate(this, ' + data[0] + ')">' +
					//		'<i class="fa fa-check-circle" aria-hidden="true"></i> Activate' +
					//		'</button>';
					//} else {
					//	actions += '<button type="button" class="btn btn-sm mr-1' + status[isActive].class + '" onclick="Activate(this, ' + data[0] + ')">' +
...
function Delete(element, record) {
	...
	swal.fire({
		...
		if (result.value) {
			...
			$.ajax({
				url: '/Admin/ClientTypes/Delete/' + record,
				type: 'POST',
				data: {
				...
				success: function (result) {
					if (result.success != undefined) {
						if (result.success) {
							...
							toastr.success('Client type deleted successfully ...');
...
function Activate(element, record) {
	swal.fire({
		...
		if (result.value) {
			...
			$.ajax({
				url: '/Admin/ClientTypes/ToggleActiveStatus?CityId=' + record,
				type: 'Get',
