...
namespace WebAPI.Controllers.Admin
{
    ...
    public class AdminAccountController : ControllerBase
    {
        ...
        private readonly IUserService _userServiceCustom;

        private readonly Uri _webAppUrl;
        private IWebHostEnvironment _hostingEnvironment;

        public AdminAccountController(SignInManager<AppUser> signInManager, UserManager<AppUser> userManager, IConfiguration config,
            ILogger<AdminAccountController> logger, IMapper mapper, ITokenService tokenService, IEmailService emailService,
            IUserRefreshTokenService refreshTokenService, IRouteGroupService routeGroupService, IWebHostEnvironment hostingEnvironment,
            IGarageService garageService, IUserService userService, ISparePartsDealerService sparePartsDealerService,
            IMessageService messageService, IRestaurantService restaurantService, IUserService userServiceCustom)
        {
            ...
            _userServiceCustom = userServiceCustom;
        ...
        {
            IEnumerable<AppUser> AdminUsers = await _userServiceCustom.GetUserByEmailAndCheck(model.Email, Enum.GetName(typeof(Logins), Logins.Admin));
            IEnumerable<AppUser> RestaurantUsers = await _userServiceCustom.GetUserByEmailAndCheck(model.Email, Enum.GetName(typeof(Logins), Logins.Restaurant));

            if (!AdminUsers.Any() && !RestaurantUsers.Any())
                return Conflict(new ErrorDetails(409, "No User Found, Invalid Email", string.Empty));

            AppUser User = AdminUsers.Any() ? AdminUsers.FirstOrDefault() : RestaurantUsers.FirstOrDefault();

            if (!User.IsActive)
            {
                return Conflict(new ErrorDetails(409, "Account Suspended!", string.Empty));
            }
            if (User.IsDeleted)
            {
                return Conflict(new ErrorDetails(409, "No User Found!", string.Empty));
            }
            ...
            if (result.Succeeded)
            {
                ...
                if (User.LoginFor == Enum.GetName(typeof(Logins), Logins.Restaurant)) { }
                    Response.Restaurant = _mapper.Map<IEnumerable<RestaurantDTO>>(await _restaurantService.GetRestaurantByUserAsync(User.Id)).FirstOrDefault();
            ...
            return Conflict(new ErrorDetails(401, "Invalid email or password", string.Empty));
        }
        ...
        public async Task<bool> IsPhoneNumberConfirmed(string PhoneNumber)
        {
            ...
            {
                var users = await _userService.GetUserByNumberAndCheck(PhoneNumber, Enum.GetName(typeof(Logins), Logins.Admin));
        ...
        {
            ErrorDetails err;
            IEnumerable<AppUser> Users = await _userService.GetUserByNumberAndCheck(PhoneNumber, Enum.GetName(typeof(Logins), Logins.Admin));
        ...
        public async Task<IActionResult> Register(RegisterData Model)
        {
            ...
            if (result.Succeeded)
            {
                await _userManager.AddToRoleAsync(User, Enum.GetName(typeof(Roles), Roles.Admin));
        ...
        {
            Random rnd = new();
            var adminusers = await _userService.GetUserByEmailAndCheck(Email, Enum.GetName(typeof(Logins), Logins.Admin));
            var restaurantusers = await _userService.GetUserByEmailAndCheck(Email, Enum.GetName(typeof(Logins), Logins.Restaurant));

            if (!adminusers.Any() && !restaurantusers.Any())
                return Conflict(new ErrorDetails(409, "User not found", string.Empty));

            var user = adminusers.Count() != 0 ? adminusers.FirstOrDefault() : restaurantusers.FirstOrDefault();
            //var user = await _userManager.FindByEmailAsync(Email);

            if (user == null)
            {
                _logger.LogError("Forget Password: Unable to find " + Enum.GetName(typeof(Logins), Logins.Admin) + " user " + Email);
                return Conflict(new ErrorDetails(409, "User not found", string.Empty));
            }

            if (!(await _userManager.IsEmailConfirmedAsync(user)))
            {
                _logger.LogError("Forget Password: " + Enum.GetName(typeof(Logins), Logins.Admin) + " User phone number is not confirmed " + user.Email);
                return Conflict(new ErrorDetails(409, "Email is not confirmed", string.Empty));
            }
        ...
        public async Task<IActionResult> ResetPassword(ChangePasswordDTO Model)
        {
            ...
            {
                _logger.LogError("New Password is empty" + Model.UserId);
                return Conflict(err = new ErrorDetails(409, "Please fill the new password field", string.Empty));
            }
