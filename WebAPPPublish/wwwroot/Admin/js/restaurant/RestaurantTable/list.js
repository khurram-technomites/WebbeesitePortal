        ...
        table1 = table.DataTable({
            ...
            columnDefs: [{
                ...
                render: function (data, type, full, meta) {
                    ...
                    actions += '<button type="button" class="btn btn-bg-secondary btn-icon btn-sm mr-1" onclick="OpenModelPopup(this,\'/Restaurant/RestaurantTable/Edit/' + data[1] + '\',true)">' +
                        '<i class="fa fa-pen"></i>' +
                        '</button> ' +
                        '<button type="button" class="btn btn-bg-secondary btn-icon btn-sm mr-1" onclick="OpenModelPopup(this,\'/Restaurant/RestaurantTable/Details/' + data[1] + '\')" title="View">' +
...

            {
            ...
            },
                {
                    ...
                    render: function (data, type, full, meta) {
                        ...
                        var tableIcon = '/Images/Icons/tableIcon.png';


                        return '<div class="d-flex align-items-center">' +
                            '<div class="symbol symbol-35 flex-shrink-0 mr-4">' +
                            '<div class="symbol-label" style="background-image: url(\'' + tableIcon + '\')"></div>' +
                            '</div>' +
                            '<div>' +
                            '<a href="#" class="text-dark-75 font-weight-bolder text-hover-primary mb-1 font-size-lg">' + table[0] + '</a><br>' +
                            // '<a href="#" class="text-dark-75 font-weight-bolder text-hover-primary mb-1 font-size-lg">' + vendor[3] + '</a>' +
                            '<small class="text-muted font-weight-bold d-block">' + table[1] + ' Person </small>' +
                            '</div>' +
                            '</div>';
...
function Delete(element, record) {
    ...
    swal.fire({
        ...
        if (result.value) {
            ...
            $.ajax({
                url: '/Restaurant/RestaurantTable/Delete/' + record,
                type: 'POST',
                data: {
...
function Activate(element, record) {
    swal.fire({
        ...
        if (result.value) {
            ...
            $.ajax({
                url: '/Restaurant/RestaurantTable/ToggleActiveStatus/' + record,
                type: 'Get',
