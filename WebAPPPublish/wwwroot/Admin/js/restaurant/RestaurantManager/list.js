			...
			"initComplete": function (settings, json) {
			 ...
			 columnDefs: [{
                ...
                render: function (data, type, full, meta) {
					...
					actions += '<button type="button" class="btn btn-bg-secondary btn-icon btn-sm mr-1" onclick="OpenModelPopup(this,\'/Restaurant/RestaurantManager/Edit/' + data[1] + '\',true)">' +
                        '<i class="fa fa-pen"></i>' +
                        '</button> ' +
                        '<button type="button" class="btn btn-bg-secondary btn-icon btn-sm mr-1" onclick="OpenModelPopup(this,\'/Restaurant/RestaurantManager/Details/' + data[1] + '\')" title="View">' +
...
function Delete(element, record) {
	...
	swal.fire({
		...
		if (result.value) {
			...
			$.ajax({
				url: '/Restaurant/RestaurantManager/Delete/' + record,
				type: 'POST',
				data: {
...
function Activate(element, record) {
	swal.fire({
		...
		if (result.value) {
			...
			$.ajax({
				url: '/Restaurant/RestaurantManager/ToggleActiveStatus/' + record,
				type: 'Get',
