...
var model = {
	SparePartBusinessSettingId: null,
	Id: null,
	Name: null,
...
function Save(element) {
	...
	if (row.find(".Name").val() == "") {
		...
		data.append("SparePartBusinessSettingId", BusinessSettingID);
		...
		$.ajax({
			url: "/SparePart/SparePartBusinessSetting/BranchSettings/",
			type: "POST",
			processData: false,
			contentType: false,
			data: data,
...
function Delete(element, record) {
	...
	if (record == 0) {
		...
		swal.fire({
			...
			if (result.value) {
				$.ajax({
					url: '/SparePart/SparePartBusinessSetting/DeleteBranchSetting/' + record,
					type: 'POST',
					data: {
					...
					success: function (response) {
						if (response.success != undefined) {
							...
							swal.fire("Your are not authorize to perform this action", "For further details please contact SparePartistrator !", "warning").then(function () {
							});
						}
					},
					error: function (xhr, ajaxOptions, thrownError) {
						if (xhr.status == 403) {
							try {
								...
								swal.fire("Access Denied", "Your are not authorize to perform this action, For further details please contact SparePartistrator !", "warning").then(function () {
									$('#myModal').modal('hide');
								});
							}
