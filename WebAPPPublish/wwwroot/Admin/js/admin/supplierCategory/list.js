		...
		table1 = table.DataTable({
			...
			columnDefs: [{
				...
				render: function (data, type, full, meta) {
					...
					actions += '<button type="button" class="btn btn-bg-secondary btn-icon btn-sm mr-1" onclick="OpenModelPopup(this,\'/Admin/SupplierCategory/Edit/' + data + '\',true)">' +
						'<i class="fa fa-pen"></i>' +
						'</button> ' +
						'<button type="button" class="btn btn-bg-secondary btn-icon btn-sm mr-1" onclick="OpenModelPopup(this,\'/Admin/SupplierCategory/Details/' + data + '\')" title="View">' +
...
function Delete(element, record) {
	...
	swal.fire({
		...
		if (result.value) {
			...
			$.ajax({
				url: '/Admin/SupplierCategory/Delete/' + record,
				type: 'POST',
				data: {
				...
				success: function (result) {
					if (result.success != undefined) {
						if (result.success) {
							...
							toastr.success('Supplier Category deleted successfully ...');
...
function Activate(element, record) {
	swal.fire({
		...
		if (result.value) {
			...
			$.ajax({
				url: '/Admin/SupplierCategory/ToggleActiveStatus?Id=' + record,
				type: 'Get',
