			...
			columnDefs: [
				...
				//},
				{
					...
					render: function (data, type, full, meta) {
						...
						if (data) {
							var actions = '<button type="button" class="btn btn-outline-success btn-sm mr-1" onclick="OpenModelPopup(this,\'/Admin/Coupon/GetCustomers/' + data + '\',true)">' +
...
function Activate(element, record) {
	swal.fire({
		...
		if (result.value) {
			...
			$.ajax({
				url: '/Admin/Coupon/ToggleActiveStatus/' + record,
				type: 'Get',
