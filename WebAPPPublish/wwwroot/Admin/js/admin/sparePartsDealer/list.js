            ...
            columnDefs: [
                {
                    ...
                    render: function (data, type, full, meta) {
                        ...
                        actions += '<a type="button" class="btn btn-bg-secondary btn-icon btn-sm mr-1"  href="/Admin/SparePartsDealer/Edit/' + data + '" >' +
                            '<i class="fa fa-pen"></i>' +
                            '</a> ' +
                            '<a type="button" class="btn btn-bg-secondary btn-icon btn-sm mr-1" href="/Admin/SparePartsDealer/Detail/' + data + '" title="View">' +
...
function Delete(element, record) {
    ...
    swal.fire({
        ...
        if (result.value) {
            ...
            $.ajax({
                url: '/Admin/SparePartsDealer/Delete/' + record,
                type: 'POST',
                data: {
...
function activeChange(element, flag, id) {
    swal.fire({
        ...
        if (result.value) {
            ...
            $.ajax({
                url: '/Admin/SparePartsDealer/ToggleActiveStatus?Id=' + id + '&flag=' + flag,
                type: 'Get',
...
function Activate(element, record) {
    swal.fire({
        ...
        if (result.value) {
            ...
            $(element).addClass('spinner spinner-left spinner-sm').attr('disabled', true);
            $.ajax({
                url: '/Admin/SparePartsDealer/ToggleActiveStatus/' + record,
                type: 'Get',
                success: function (response) {
                    if (response.success) {
...
function Reject(element, flag, id) {
    Swal.fire({
        ...
        if (result.value) {
            ...
            $.ajax({
                url: '/Admin/SparePartsDealer/ToggleActiveStatus?Id=' + id + '&flag=' + flag + '&RejectionReason=' + result.value,
                type: 'Get',
                success: function (response) {
                    if (response.success) {
                        toastr.success(response.message);
