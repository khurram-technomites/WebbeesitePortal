            ...
            columnDefs: [
                {
                    ...
                    render: function (data, type, full, meta) {
                        ...
                        var actions = '';
                        /*FOR EDIT ------>         /' + data + '*/
                        actions += '<a class="btn btn-bg-secondary btn-icon btn-sm mr-1" href="/Admin/Garage/Edit" title="Edit">' +
                            '<i class="fa fa-pen"></i>' +
                            '</a> ';
...
function Delete(element, record) {
    ...
    swal.fire({
        ...
        if (result.value) {
            ...
            $.ajax({
                url: '/Admin/Garage/Delete/' + record,
                type: 'POST',
                data: {
...
function Activate(element, record) {
    swal.fire({
        ...
        if (result.value) {
            ...
            $.ajax({
                url: '/Admin/Garage/ToggleActiveStatus/' + record,
                type: 'Get',
                success: function (response) {
                    if (response.success) {
                        toastr.success(response.message);
